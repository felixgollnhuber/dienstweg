...
4. **Redundant review.** N independent reviewers (default 3), run in parallel, with an identical, broad scope - review subagents in Claude Code, separate `codex exec` processes in Codex. Each reviewer takes a distinct stance (`review.stances`, default `adversarial` / `spec-conformance` / `maintainer`, assigned round-robin) to decorrelate the ensemble without narrowing scope. The value is the ensemble: consensus = high priority, singletons = judge critically, conflicts = decide explicitly.
...
- **Distinct stance per reviewer** - on top of the identical broad scope, each reviewer is assigned one stance from `review.stances` (default `adversarial` / `spec-conformance` / `maintainer`) round-robin (reviewer i → `stances[i % stances.length]`), leading with that stance's lens but still covering the whole diff, so the stances decorrelate the ensemble. The `spec-conformance` stance name is load-bearing: the reviewer assigned it also receives the issue reference and checks the diff against the issue's `## Plan` and `## Acceptance Criteria`; drop that stance and no reviewer performs the Plan/AC check.
