...
- **Codex delegation mode for start-task (Claude Code harness only).** `/start-task <ID> --codex` - or the optional config key `delegation.implementer: "codex"`, which sets the default (`--codex`/`--no-codex` override it per run) - makes the subsequent `/goal` loop delegate implementation steps to the `codex:codex-rescue` subagent while Claude keeps orchestrating, reviewing, and merging. A read-only preflight (`codex-companion.mjs setup --json` -> `ready: true`) runs before the issue is claimed and stops with a pointer to `/codex:setup` when the openai-codex plugin is not ready. The ensemble review keeps its configured size but swaps one Claude reviewer for a read-only Codex `adversarial-review` (adversarial stance; a configured `spec-conformance` always stays on a Claude reviewer). Fallback: one `--resume` retry per failed delegation, then Claude implements the step itself and records the fallback as an issue comment. Without the flag/config key the templates behave exactly as before; the Codex-harness start-task skill is unchanged.
- **Guard decision log + `check` surfacing.** Every branch-guard rule now carries a stable id, and each block / warn / fail-open decision appends one JSON line to `.dienstweg/guard-log.jsonl` (best-effort and silent — a write failure never changes the guard's block/allow decision or its exit code, so logging can never make the guard fail open). `dienstweg check` summarizes recent blocks per rule as an INFO and, critically, reports recent **fail-open** events (the guard fell open because the config was unreadable at hook runtime, so a violation may have slipped through) as a FAIL — an inert guard is loud instead of silent. Bounded to a 7-day recency window, so the FAIL self-heals once events age out. `init`/`update` gitignore the log; it is a per-machine debug artifact, never committed.
