...
- Config schema v3: `dienstweg update` adds `harnesses` (default both) to existing repos, so a pre-v3 Claude-only repo gains the full Codex surface on the next update. `check`, the onboarding audit, and the manifest are all harness-aware - they verify/scan only the active harnesses and flag pre-existing `.codex/` and `.agents/skills/` collisions.
