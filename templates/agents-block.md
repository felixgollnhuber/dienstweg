...
**Codex delegation mode (Claude Code harness only):** `/start-task {{issuePrefix}}-XXX --codex` - or the optional config key `delegation.implementer: "codex"` in `dienstweg.config.json`, which sets the default (a flag wins: `--codex` forces ON, `--no-codex` forces OFF for one run) - makes the subsequent `/goal` loop delegate the implementation steps to the `codex:codex-rescue` subagent while Claude keeps orchestrating, reviewing, and merging. Preflight before the issue is claimed: the openai-codex plugin must report ready (`codex-companion.mjs setup --json` -> `ready: true`), otherwise start-task stops and points to `/codex:setup`. The ensemble review keeps its size ({{ensembleSize}} reviewers) but one reviewer is Codex - with the default config 2x Claude + 1x Codex: the Codex reviewer runs read-only (`adversarial-review`) and takes the `adversarial` stance when configured, otherwise the first non-spec-conformance stance; a configured `spec-conformance` (Plan/AC check + issue reference) always lands on a Claude reviewer. Fallback: a failed delegation gets exactly one retry via `--resume`, after which Claude implements the step itself and records the fallback as an issue comment. The Codex-harness start-task skill does not have this mode.
...
**Review (mandatory before every merge):** {{ensembleSize}}x ensemble review - {{ensembleSize}} independent reviewers run in parallel, no splitting. Every reviewer keeps the **same identical, broad scope** (the full PR) but takes a **distinct stance** to decorrelate the ensemble: stances {{stances}}, assigned round-robin across the {{ensembleSize}} reviewers. When a `spec-conformance` stance is configured, the reviewer assigned it additionally receives the issue reference and checks the diff against the issue's `## Plan` and `## Acceptance Criteria` (that stance name is load-bearing - it keys the Plan/AC check; drop it and no reviewer performs that check). The fan-out mechanism is harness-specific (Claude Code: parallel review subagents in one message; Codex: parallel `codex exec` reviewers) - see the start-task command. Fix consensus findings directly, judge singletons critically, decide conflicts explicitly. Re-review on larger fix changes (new logic / high-risk / >50 LOC / >3 new files / interface change), max {{maxRounds}} rounds. A single review pass does not replace the ensemble.
