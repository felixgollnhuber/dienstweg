---
name: start-task
description: Prepare a Linear issue for implementation - worktree, a comprehensive plan written into the issue, and a ready-to-run /goal condition. Use when the user wants to start, begin, pick up, or work on an existing issue / ticket by its identifier (e.g. "start FAC-123"). Sets up only; implementation runs in the /goal loop afterwards.
---

You are preparing the Linear issue the user named (an identifier like `123` or `PREFIX-123`) for implementation. Goal: a worktree + a comprehensive plan in the Linear issue description (section `## Plan`), so that `/goal` can afterwards work through it without further questions.
...
- Read `dienstweg.local.md` if it exists - its rules apply in addition to this skill.
...
- Normalize the identifier to the format `<issuePrefix>-XXX` (e.g. `123` -> `ABC-123` when the prefix is ABC).
...
- **Claim now** (before planning, to close the single-writer race window): `save_issue state="In Progress" assignee="me"`. If the issue is already `In Progress` and assigned to someone else, STOP and ask.
...
- Worktree path: `.codex/worktrees/tasks+<issueprefix-lowercase>-XXX-<slug>` relative to the main repo (or the location the project defines in `dienstweg.local.md`).
...
```
git worktree add .codex/worktrees/tasks+<prefix>-XXX-<slug> -b tasks/<prefix>-XXX-<slug> <config.git.baseBranch>
cd .codex/worktrees/tasks+<prefix>-XXX-<slug> && <project setup, e.g. npm ci - skip if not applicable>
```
...
## Step 4 - Ask questions, then plan

- Ask as many questions as needed to resolve ambiguities, **one at a time**. **Two extra questions beat one silent assumption.** Typical topics:
...
- Do NOT write to Linear yet - compose the plan in memory now and persist it only after the user approves (step 8).

## Step 5 - Compose the comprehensive plan (in memory, do not write yet)

The plan must be `/goal`-ready. It goes into the Linear issue description's `## Plan` block after approval (step 8). Required sections:
...
**Touch points are amendable mid-task.** The `## Plan` block is not frozen at approval. If the `/goal` loop later finds it insufficient, it uses the **amendment move** (defined in the AGENTS block): append an `### Amendments` sub-section to `## Plan` via `save_issue`, each entry recording reason, new touch points, and a LOC estimate. Autonomous only when the amendment adds `<=2` new non-high-risk touch points (a touch point is any file that must change but was not in the original plan, created or edited); if it adds `>2` new touch points, or any high-risk area (`config.areas.highRisk`), stop and ask first. That is why the `/goal` file-scope constraint below reads "amended plan" - a recorded amendment is not a violation, whereas a genuine scope change still means ask or a follow-up issue.

## Step 6 - Compose the /goal condition

`/goal` is a Codex built-in: it keeps working toward one durable objective until the stopping condition is reached, so the condition needs **measurable, transcript-verifiable end states**. Compose it exactly after this schema (single line, in `config.language`, ready to send - replace every `<...>` with the concrete values from the config and this issue):

```
/goal <issuePrefix>-XXX plan fully implemented: the approved ## Plan block written to the issue description via save_issue, all implementation steps from that ## Plan block done (if the plan proves insufficient mid-task, amend it first - append `### Amendments` to ## Plan via save_issue with reason + new touch points + LOC estimate; autonomous only for <=2 new non-high-risk touch points, otherwise stop and ask), <config.gates.build> exit 0, all acceptance criteria boxes in the ## Acceptance Criteria section toggled to `- [x]` via save_issue, PR created against <config.git.baseBranch> (title: "<issuePrefix>-XXX - <title>"), <config.review.ensembleSize>x ensemble review executed (<config.review.ensembleSize> independent reviewers run in parallel as separate `codex exec` processes in ONE batch, broad scope) with consensus synthesis and follow-up commits for consensus findings, re-review loop on larger changes (threshold: new logic / high-risk / >50 LOC / >3 new files / interface change, max <config.review.maxRounds> rounds), BEFORE `gh pr merge` all DoD boxes in the ## Definition of Done section toggled to `- [x]` via save_issue and the ## Final Summary section set with merge-SHA placeholder + PR number plus state="In Review", auto-merge via `gh pr merge <N> --squash --delete-branch` only if all gates are green (base=<config.git.baseBranch>, build exit 0, all DoD boxes checked, no open majority critical findings, re-review loop finished, no user override), MANDATORY step after a successful merge: in the MAIN working copy (not the task worktree) run `git checkout <config.git.baseBranch> && git pull --ff-only` and confirm the new HEAD in a user message - this step must NOT be skipped, and AS THE LAST STEP BEFORE LOOP EXIT explicitly save_issue state="Done" plus a description patch with the real merge SHA in ## Final Summary (the loop must NOT stop earlier even if everything else looks finished). Constraints: no --no-verify, no hook bypass, no push to protected branches, no force push, <config.extraConstraints, comma-joined - omit if empty>, no files outside the amended plan's touch points. Stop after 40 turns if not fulfilled.
```

**Ensemble review via `codex exec`** (this is the Codex fan-out the condition refers to): launch `config.review.ensembleSize` reviewers as independent `codex exec` processes in ONE batch (true parallelism), each with the PR number + branch + worktree path + an identical broad-scope prompt (code quality, bugs, logic, conventions, tests, edge cases, error handling, security, high-risk areas, performance) and asked for a structured report (Critical / Important / Suggestions / Strengths) with `file:line` references. Synthesize yourself: consensus findings (>=2 reviewers) fixed directly, singletons judged critically, conflicts decided explicitly. Prefer `codex exec --json` for machine-readable output; run reviewers read-only and apply fixes yourself in the worktree afterwards.
...
Present a compact overview and get explicit approval:
...
## Step 8 - After plan approval

Once the user approves the plan: **do NOT start any implementation work yourself.** Instead:
...
Wait for the user's `/goal` command. Only when it arrives does implementation begin - through the official `/goal` loop, not this skill.

Rationale: a skill cannot launch `/goal` itself. The `/goal` loop beats a DIY continue: it keeps working toward the measurable stopping condition, is robust against intermediate errors, and can be paused/resumed.
