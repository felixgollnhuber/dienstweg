---
name: create-issue
description: Draft a new Linear issue following the dienstweg project schema - interference check plus collaborative drafting. Use when the user wants to create, open, or file a backlog issue / ticket / task for this repository. Creates ONLY the backlog issue; no code, branch, or worktree.
---

You are creating a new Linear issue about the topic the user gave you (the text of their request; if none was given, ask for it - do not guess). Goal: a complete, cleanly planned issue following the project convention (see the dienstweg block in AGENTS.md) - description template, AC, DoD, correct labels, project assignment - without conflicting with issues that are currently in flight.

**Hard rule up front**: this skill changes **no code**, creates **no branch**, initializes **no worktree** and sets **no state to "In Progress"**. All of that happens later via the `start-task` skill. Here we only create a backlog issue.
...
- If no topic was provided: ask the user for it (a title idea + 1-3 sentences of context). Do not guess.
- If the topic is a single vague keyword: ask what concretely should happen **before** starting step 2. An interference check without a clear scope is wasted time.
...
Goal: find issues that could collide with the planned topic - by content, by shared files/modules, or through single-writer locks. Run **all** of these checks (batch them):
...
If an existing issue **already covers** the topic: STOP, show the user the hits and ask whether the old issue should be used/extended instead, or the new issue is still a separate scope.

## Step 4 - Draft together (present, do not create yet)

- First present the **conflict table** compactly (even when empty - then say explicitly "no interference found").
- Then ask as many questions as needed, **one at a time**. **Two or three extra questions beat one silent assumption.** Typical topics:
...
Before creating: show the user the **full body + metadata** (title, project, milestone, labels, priority, relations) and get explicit approval. On change requests go back to step 4 - no silent adjustments.

## Step 6 - After approval: create the issue
...
Next step (optional):
Run the start-task skill on <issuePrefix>-XXX - create worktree + plan for implementation.
```

If the conflict table had hits and the user decided to create the issue anyway: list the conflicts again at the end with the reasoning.

## Hard rules

- No code changes, no branch/worktree creation, no build/setup runs in this skill. Pure Linear MCP drafting.
...
- Batch independent tool calls (steps 2 + 3).
